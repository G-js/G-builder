...
function Builder(config) {
    ...
    this.config = config;
    this.extraConfig = {};
    this.db = new Database({filename: config.db || './data/fileinfo.db', autoload: true});
}

util.inherits(Builder, EventEmitter);

Builder.prototype.getFileList = function (input, callback) {
    var self = this;
...
Builder.prototype.build = function (input, callback) {
    var self = this;
    ...
    this.getFileList(input, function (err, files) {
...
function getRelatedFiles (searchList, db, callback) {
    ...
    async.whilst(
        ...
        function (cb) {
            ...
            db.find({children: filename}, function (err, docs) {
                if (err) {
                    return cb(err);
                }
