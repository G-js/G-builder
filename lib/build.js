...
Builder.prototype.checkUpdate = function (callback) {
...
Builder.prototype.getVersions = function (callback) {
    var expire = this.config.expire;
    var deadLine = 0;
    var now = Date.now() / 1000;
    if (expire) {
        deadLine = now - now % expire;
    }

    this.db.find({last_update: {$gt: deadLine}}, function (err, docs) {
        if (err) {
            return callback(err);
        }

        var versions = docs.reduce(function (versions, doc) {
            versions[doc.filename] = doc.last_update;
            return versions;
        }, {});
...
Builder.prototype.build = function (input, options, callback) {
            ...
            function (report, file, next) {
                ...
                stream.run(context, function (err, fileInfo) {
                    ...
                    if (err) {
                        ...
                        self.emit('fail', file, err);
                        return next(new Error('Error in building [' + file + ']: ' + (err.message || err)), report);
                    }
                    self.updateFileInfo({
    ...
    async.waterfall(
        ...
        function (err, report) {
            ...
            self.getVersions(function (err, versions) {
                if (err) {
                    return callback(new Error('fail to generate versions'), report);
                }
                mkdirp(self.config.dest, function (err) {
                    if (err) {
                        return callback(new Error('fail to mkdir: ' + self.config.dest));
                    }

                    fs.writeFile(self.config.dest + 'g-version.json', JSON.stringify(versions, null, 4), function (err) {
...
Builder.prototype.updateFileInfo = function (fileInfo, timestamp, callback) {
    var db = this.db;

    db.update({filename: fileInfo.filename}, fileInfo, {upsert: true}, function (err) {
        if (err) {
            return callback(err);
        }
...
Builder.prototype.watch = function (onReady, onSuccess, onFail) {
    var self = this;
